...
import org.shaper.swagger.model.TestInput
...
class SimpleInputGenertor(
    ...
    fun getInput(endpoint: EndpointSpec): List<TestInput> {
    ...
    private fun getParamVals(spec: ParameterSpec, numVals: Int = 5): List<*> {
        return when (spec.dataType) {
            ...
            Int::class -> (1..numVals).toList().map { Faker().number().numberBetween(-100, 100) }
